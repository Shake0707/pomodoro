...
const ShortBr: FC = () => {
    ...
    const { isPause, seconds, myDeg } = timers.shortBreak;
    ...
    useEffect(() => {
        if (!isPause) {
            myInterval = setInterval(() => {
                dispatch(increment({ type: 'shortBreak' }));
            }, 1000)
        } else {
    ...
    useEffect(() => {
        ...
        dispatch(changeDeg({ type: "shortBreak" }));
    }, [seconds]);

    const handleClick: MouseEventHandler<HTMLDivElement> = () => {
        dispatch(toggleIsPause({ type: "shortBreak" }));
        start();
    }
    ...
    return (
        <div className={classes.container}
            style={{
                display: mode === 'short break' ? "block" : "none"
            }}
            onClick={handleClick}
        >
